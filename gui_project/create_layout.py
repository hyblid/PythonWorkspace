...
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요", filetypes=(("PNG 파일", "*.png"), ("모든 파일", "*.*")),
                                        initialdir=r"C:\test\PythonWorkspace")  # 최초에 C:/ 경로를 보여줌
    # 사용자가 선택한 파일 목록
    for file in files:
...
def merge_image():
    ...
    try:
        ...
        image_size = []
        if image_width > -1:
            image_size = [(int(image_width), int(
                image_width * x.size[1]/x.size[0])) for x in images]
        else:
            image_size = [(x.size[0], x.size[1]) for x in images]

        # siz -> siz[0] : width, size[1] : height
        ...
        # 포맷 옵션 처리
        file_name = "nado_photo." + image_format
...
def start():
    ...
    # 파일 목록 확인
    if list_file.size() == 0:
        msgbox.showerror("경고", "이미지 파일을 추가하세요")
        return

    # 저장 경로 확인
    if len(txt_dest_path.get()) == 0:
        msgbox.showerror("경고", "저장 경로를 선택하세요")
        return
...
current_work_path = os.path.dirname(__file__)
txt_dest_path.insert(END, current_work_path)
...
# 가로 넓이 콤보
cmb_width = ["원본유지", "1024", "800", "640"]
cmb_width = ttk.Combobox(frame_option, state="readlonly",
                         values=cmb_width, width=10)
cmb_width.current(0)
...
# 가로 넓이 콤보
cmb_space = ["없음", "좁게", "보통", "넓게"]
cmb_space = ttk.Combobox(frame_option, state="readlonly",
                         values=cmb_space, width=10)
cmb_space.current(0)
...
# 가로 넓이 콤보
cmb_format = ["PNG", "JPG", "BMP"]
cmb_format = ttk.Combobox(
    frame_option, state="readlonly", values=cmb_format, width=10)
cmb_format.current(0)
cmb_format.pack(side="left", padx=5, pady=5)

# 진행 상황 Progress Bar
frame_progess = LabelFrame(root, text="진행 상황")
frame_progess.pack(fill="x", padx=5, pady=5, ipady=5)  # 간격 띄우기)
p_var = DoubleVar()
...
root.resizable(False, False)  # x(width), y(height) 값 변경 불가 (창 크기 변경 불가)
root.mainloop()