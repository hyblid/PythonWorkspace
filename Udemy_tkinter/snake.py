...
class Game(ctk.CTk):
    def __init__(self):
        ...
        #snake coordinate (ROW, COLUMN) - [(5,7),(4,7),(3,7)]
    ...
    def animate(self):
        #snake update
        #We have to create a new head = old head + direction
    ...
    def draw(self):
        if self.draw_frames:
            #empty the window
            #1. grid_forget all frames in the draw frames list
            for frame, pos in self.draw_frames:
                frame.grid_forget()
            #2. emtpy the draw frams list
